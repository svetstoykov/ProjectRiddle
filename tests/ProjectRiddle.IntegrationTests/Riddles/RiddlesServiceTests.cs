...
public sealed class RiddlesServiceTests
{
    ...
    public async Task TodayRequiresAPublishedRiddleOnTheLocalDate()
    {
        ...
        var missing = await workspace.Service.GetTodayAsync(CancellationToken.None);
        Assert.True(missing.IsFailure);
        Assert.Equal(RiddleErrorCodes.TodayUnavailable, missing.Error!.Code);

        var created = await workspace.AdminService.CreateAsync(TestWorkspace.CreateRiddleInput(), CancellationToken.None);
        ...
        var stillMissing = await workspace.Service.GetTodayAsync(CancellationToken.None);
    ...
    public async Task LocalDateRolloverTurnsTodayIntoArchive()
    {
        ...
        var anonymousPlay = await workspace.Service.GetPlayAsync(published.Id, CancellationToken.None);
        ...
        var authenticated = await workspace.Service.GetPlayAsync(published.Id, CancellationToken.None);
    ...
    public async Task AnswersAreNormalizedAndTerminalRetriesAreIdempotent()
    {
        ...
        var incorrect = await workspace.Service.SubmitAnswerAsync(
        ...
        var retry = await workspace.Service.SubmitAnswerAsync(
    ...
    public async Task StructuralHintsAreRecordedOnce()
    {
        ...
        var first = await workspace.Service.UseHintAsync(
            ...
            CancellationToken.None);
        var repeat = await workspace.Service.UseHintAsync(
            new UseRiddleHintInput(published.Id, RiddleRangeKind.Definition, null),
            CancellationToken.None);
        var indicator = await workspace.Service.UseHintAsync(
    ...
    public async Task LetterRevealsAreUniqueAndSolvedTakesPrecedence()
    {
        ...
        var first = await workspace.Service.RevealLetterAsync(
        ...
        var second = await workspace.Service.RevealLetterAsync(
            new RevealRiddleLetterInput(published.Id, null),
            CancellationToken.None);
        ...
        {
            var revealed = await remainingWorkspace.Service.RevealLetterAsync(
        ...
        var solved = await remainingWorkspace.Service.SubmitAnswerAsync(
    ...
    public async Task AnonymousResumeValidatesAndRehydratesPermittedLetters()
    {
        ...
        var invalid = await workspace.Service.ResumeAsync(
    ...
    public async Task ImportMergesMonotonicallyAndRejectsDeletedRiddles()
    {
        ...
        var first = new AnonymousRiddleProgressInput(
            ...
            [0, 1, 2, 3, 4, 5, 6, 7, 8]);
        var imported = await workspace.Service.ImportProgressAsync(first, CancellationToken.None);
        ...
        var merged = await workspace.Service.ImportProgressAsync(stale, CancellationToken.None);
        ...
        var solvedImport = await workspace.Service.ImportProgressAsync(
        ...
        var unpublished = await workspace.AdminService.UnpublishAsync(published.Id, CancellationToken.None);
        var deleted = await workspace.AdminService.DeleteAsync(published.Id, CancellationToken.None);
        Assert.True(unpublished.IsSuccess);
        Assert.True(deleted.IsSuccess);

        var afterDelete = await workspace.Service.ImportProgressAsync(first, CancellationToken.None);
    ...
    public async Task InvalidPlayCommandsAreRejected()
    {
        ...
        var empty = await workspace.Service.SubmitAnswerAsync(
        ...
        var hint = await workspace.Service.UseHintAsync(
    ...
    {
        var workspace = new TestWorkspace(NoonUtcOnTwentieth);
        var missing = await workspace.Service.GetPlayAsync(Guid.NewGuid(), CancellationToken.None);
        Assert.True(missing.IsFailure);
        Assert.Equal(RiddleErrorCodes.NotFound, missing.Error!.Code);

        var created = await workspace.AdminService.CreateAsync(TestWorkspace.CreateRiddleInput(), CancellationToken.None);
        var draft = await workspace.Service.GetPlayAsync(created.Value!.Id, CancellationToken.None);
        ...
        var futurePlay = await workspace.Service.GetPlayAsync(future.Id, CancellationToken.None);
    ...
    {
        var workspace = new TestWorkspace(NoonUtcOnTwentieth);
        var invalidPage = await workspace.Service.ListArchiveAsync(
        ...
        workspace.Account.AccountId = AccountId;
        var progress = await workspace.Service.ListProgressAsync(
    ...
    public async Task AccountProgressReadsAreBoundedAndAuthenticated()
    {
        ...
        var unauthenticated = await anonymous.Service.ListProgressAsync(
            new ListAccountRiddleProgressInput(Today, Today),
            CancellationToken.None);
        ...
        var workspace = new TestWorkspace(NoonUtcOnTwentieth, AccountId);
        var inverted = await workspace.Service.ListProgressAsync(
        ...
        var listed = await workspace.Service.ListProgressAsync(
            new ListAccountRiddleProgressInput(Today, Today),
            CancellationToken.None);
